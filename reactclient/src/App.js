...
function App() {
  ...
  const [showingCreateNewGameForm, setShowingCreateNewGameForm] =
    useState(false)
  const [gameCurrentlybeingUpdated, setGameCurrentlybeingUpdated] =
    useState(null)
    ...
    <div className="container">
      <div className="row min-vh-100">
        <div className="col d-flex flex-column justify-content-center align-items-center">
          {showingCreateNewGameForm === false &&
            gameCurrentlybeingUpdated === null && (
              <div>
                <h1 style={{ textAlign: "center" }}>Video games</h1>
                <div className="mt-5">
                  <button
                  ...
                  <button
                    onClick={() => setShowingCreateNewGameForm(true)}
          ...
          {videoGames.length > 0 &&
            showingCreateNewGameForm === false &&
            gameCurrentlybeingUpdated === null &&
            renderVideoGamesTable()}

          {showingCreateNewGameForm && (
            <GameCreateForm onGameCreate={onGameCreate} />
          )}

          {gameCurrentlybeingUpdated !== null && (
            <GameUpdateForm
              game={gameCurrentlybeingUpdated}
          ...
          <tbody>
            {videoGames.map((game) => (
              <tr key={game.id}>
                ...
                <td>
                  <button
                    onClick={() => setGameCurrentlybeingUpdated(game)}
                  ...
                  <button
                    onClick={() => {
                      if (
                        window.confirm(
                          `Are you sure you want to delete the game titled "${game.title}" ?`
                        )
                      )
                        deleteGame(game.id)
                    }}
  ...
  function onGameCreate(createdGame) {
    ...
    alert(
      `Game successfully created. After clicking OK, your new Game title "${createdGame.title}" will show up in the table below`
    )
  ...
  function onGameUpdated(updatedGame) {
    ...
    alert(
      `Game successfully updated. After clicking OK, look for the post with the title "${updatedGame.title}" in the table below to see the updates.`
    )
  }
  ...
  function onGameDeleted(deletedGameId) {
    ...
    alert(
      "Game successfully deleted. After clicking ok, look at the table below disapear."
    )
  }
}
